...
export default function Dashboard() {
  ...
  const { data, isFetching, error, refetch } = useQuery<Chain>({
    queryKey: ['chain', symbol, strikes],
    queryFn: async () => (await api.get('/api/options/chain', { params: { symbol, strikecount: strikes } })).data,
    refetchInterval: 15000,
  })

  const track = async () => {
    await api.post('/api/ts/track', null, { params: { symbol, tracked: true } })
    alert(`Tracking ${symbol} — snapshots will start within ~60s.`)
  }
    ...
    <div>
      <div className="row" style={{ alignItems: 'center', marginBottom: 12 }}>
        <input className="input" value={input} onChange={e => setInput(e.target.value)}
               style={{ width: 260 }} placeholder="NSE:NIFTY50-INDEX" />
        ...
        <>
          ...
          <div className="row" style={{ marginTop: 12 }}>
            <div className="col" style={{ flex: 2 }}>
              <div className="card">
                <h3>Option Chain — click strike to trade</h3>
...
function OptionChainTableTradable({ chain, onTrade }: { chain: Chain; onTrade: (s: string, side: 'BUY' | 'SELL', p?: number) => void }) {
  // Lightweight trade-action overlay on top of the existing table.
  return (
    <div>
        ...
        {(() => {
          ...
          return (
            <span style={{ marginLeft: 8 }}>
              {atm.ce && <>
                <button className="primary" style={{ marginRight: 6, background: 'var(--green)', borderColor: 'var(--green)', color: '#fff' }}
                        onClick={() => onTrade(atm.ce!.symbol!, 'BUY', atm.ce!.ltp)}>BUY CE @ {atm.ce.ltp}</button>
              </>}
              {atm.pe && <>
                <button className="primary" style={{ background: 'var(--red)', borderColor: 'var(--red)', color: '#fff' }}
